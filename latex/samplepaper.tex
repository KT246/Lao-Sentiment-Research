...
From a deployment perspective, XLM-RoBERTa is the strongest choice when predictive quality is the main priority. When efficiency matters more, TextCNN offers a practical middle ground, reaching 0.9403 $F_1$-macro on the fixed split in 11.28 minutes. Within the PLM family, LoRA provides the better efficiency-quality trade-off, while SVM remains a credible low-cost baseline when infrastructure constraints dominate. Practical Lao sentiment systems should therefore be selected by balancing language fit, efficiency, and operational simplicity rather than raw accuracy alone.
...
This study establishes a benchmark for Lao sentiment classification using a filtered dataset of real-world mobile-application reviews. XLM-RoBERTa provides the strongest overall performance, while language fit and adaptation strategy remain decisive for Lao NLP. Lao-aware or stronger cross-lingual transfer outperforms generic multilingual baselines, LoRA preserves much of the PLM quality at lower training cost, and TextCNN remains a credible lightweight reference for noisy, code-mixed reviews.

Future work should expand dataset diversity, improve label consistency, and explore finer-grained settings such as aspect-level sentiment analysis. Releasing the dataset and models should support more practical Lao NLP tools.
